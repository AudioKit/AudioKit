...
open class AKAudioUnitInstrument: AKMIDIInstrument {
    ...
    open override func stop(noteNumber: MIDINoteNumber, channel: MIDIChannel, offset: UInt32 = 0) {
        guard let midiInstrument = midiInstrument else {
            AKLog("no midiInstrument exists")
            return
        }
    ...
    open override func receivedMIDIAfterTouch(_ pressure: MIDIByte, channel: MIDIChannel, offset: UInt32 = 0) {
        guard let midiInstrument = midiInstrument else {
            AKLog("no midiInstrument exists")
            return
        }
    ...
    open override func receivedMIDIPitchWheel(_ pitchWheelValue: MIDIWord, channel: MIDIChannel, offset: UInt32 = 0) {
        guard let midiInstrument = midiInstrument else {
            AKLog("no midiInstrument exists")
            return
        }
