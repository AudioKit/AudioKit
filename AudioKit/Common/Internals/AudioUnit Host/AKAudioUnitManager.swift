...
open class AKAudioUnitManager: NSObject {
    ...
    open weak var delegate: AKAudioUnitManagerDelegate? = nil {
        didSet {
    ...
    /**
     `availableEffects` is accessed from multiple thread contexts. Use
    ...
    public func requestEffects(completionHandler: (([AVAudioUnitComponent]) -> ())? = nil) {
    ...
    private func updateEffectsList( completionHandler: (() -> ())? = nil) {
        DispatchQueue.global(qos: .default).async {
            ...
            DispatchQueue.main.async {
                ...
                if self.delegate != nil {
                    self.delegate!.handleAudioUnitNotification(type: AKAudioUnitManager.Notification.effectsAvailable, object: self.availableEffects)
                }
    ...
    public func requestInstruments(completionHandler: (([AVAudioUnitComponent]) -> ())? = nil) {
    ...
    private func updateInstrumentsList( completionHandler: (() -> ())? = nil) {
        DispatchQueue.global(qos: .default).async {
            /*
             Locating components can be a little slow, especially the first time.
             Do this work on a separate dispatch thread.
            ...
            DispatchQueue.main.async {
                ...
                // notify delegate
                if self.delegate != nil {
                    self.delegate!.handleAudioUnitNotification(type: AKAudioUnitManager.Notification.instrumentsAvailable, object: self.availableInstruments)
                }
     ...
     */
    public func createEffectAudioUnit(_ componentDescription: AudioComponentDescription, completionHandler: @escaping ((AVAudioUnitEffect?) -> Void)) {
    ...
    /*
     ...
     supplied completion handler when the operation is complete.
     */
    public func createInstrumentAudioUnit(_ componentDescription: AudioComponentDescription, completionHandler: @escaping ((AVAudioUnitMIDIInstrument?) -> Void)) {
            ...
            if component.name == name {
                ...
                createEffectAudioUnit(acd, completionHandler: { au in
                    ...
                    if audioUnit.inputFormat(forBus: 0).channelCount == 1 {
                        //                        Dialog.showInformation(title: "Mono Effects aren't supported", text: "\(audioUnit.name) is a Mono effect. Please select a stereo version of it.")
                        //                        return
