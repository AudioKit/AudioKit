...
char waveNames[DRUM_NUMWAVES][16] = {
    "dope.raw",     "bassdrum.raw", "snardrum.raw", "tomlowdr.raw",
    "tommiddr.raw", "tomhidrm.raw", "hihatcym.raw", "ridecymb.raw",
    "crashcym.raw", "cowbell1.raw", "tambourn.raw"};
...
void Drummer ::noteOn(StkFloat instrument, StkFloat amplitude) {
  ...
  if (iWave == DRUM_POLYPHONY) { // This note number is not currently loaded.
    ...
    // std::cout << "iWave = " << iWave << ", nSounding = " << nSounding_ << ",
    // soundOrder[] = " << soundOrder_[iWave] << std::endl;

    // Concatenate the STK rawwave path to the rawwave file
    waves_[iWave].openFile(
        (Stk::rawwavePath() + waveNames[genMIDIMap[noteNumber]]).c_str(), true);
