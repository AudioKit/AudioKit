...
Moog ::Moog(void) {
  ...
  attacks_.push_back(
      new FileWvIn((Stk::rawwavePath() + "mandpluk.raw").c_str(), true));
  loops_.push_back(
      new FileLoop((Stk::rawwavePath() + "impuls20.raw").c_str(), true));
  loops_.push_back(new FileLoop((Stk::rawwavePath() + "sinewave.raw").c_str(),
                                true)); // vibrato
...
void Moog ::setFrequency(StkFloat frequency) {
  ...
  StkFloat rate =
      attacks_[0]->getSize() * 0.01 * baseFrequency_ / Stk::sampleRate();
