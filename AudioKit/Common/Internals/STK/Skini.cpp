...
long Skini ::nextMessage(Message &message) {
  ...
  while (!done) {
    ...
    } else if (parseString(line, message) > 0)
      done = true;
  }
...
void Skini ::tokenize(const std::string &str, std::vector<std::string> &tokens,
...
long Skini ::parseString(std::string &line, Message &message) {
  ...
  while (iSkini < __SK_MaxMsgTypes_) {
    if (tokens[0] == skini_msgs[iSkini].messageString)
      break;
    iSkini++;
  }
  ...
  while (dataType != NOPE) {
    ...
    if (tokens.size() <= (unsigned int)(iValue + 3)) {
      oStream_ << "Skini::parseString: inconsistency between type table and "
                  "parsed line:\n   "
               << line;
      handleError(StkError::WARNING);
      return message.type = 0;
