...
public:
  ...
  */
  ...
  //! Input one or two samples to the effect and return the specified \c channel
  ...
  */
  StkFloat tick(StkFloat inputL, StkFloat inputR = 0.0,
                unsigned int channel = 0);

  //! Take two channels of the StkFrames object as inputs to the effect and
  //! replace with stereo outputs.
  /*!
  ...
  */
  ...
  //! Take one or two channels of the \c iFrames object as inputs to the effect
  //! and write stereo outputs to the \c oFrames object.
  /*!
...
inline StkFloat FreeVerb::tick(StkFloat inputL, StkFloat inputR,
  ...
  for (int i = 0; i < nCombs; i++) {
    // Left channel
    // StkFloat yn = fInput + (roomSize_ *
    // FreeVerb::undenormalize(combLPL_[i].tick(FreeVerb::undenormalize(combDelayL_[i].nextOut()))));
    StkFloat yn =
        fInput + (roomSize_ * combLPL_[i].tick(combDelayL_[i].nextOut()));
    combDelayL_[i].tick(yn);
    outL += yn;

    // Right channel
    // yn = fInput + (roomSize_ *
    // FreeVerb::undenormalize(combLPR_[i].tick(FreeVerb::undenormalize(combDelayR_[i].nextOut()))));
