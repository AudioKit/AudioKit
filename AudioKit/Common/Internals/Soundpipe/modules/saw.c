...
saw* newsaw() { 
	saw* dsp = (saw*)malloc(sizeof(saw));
	return dsp;
}

void deletesaw(saw* dsp) { 
	free(dsp);
}

void instanceInitsaw(saw* dsp, int samplingFreq) {
...
void initsaw(saw* dsp, int samplingFreq) {
	instanceInitsaw(dsp, samplingFreq);
}

void buildUserInterfacesaw(saw* dsp, UIGlue* interface) {
...
void computesaw(saw* dsp, int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
...
int sp_saw_compute(sp_data *sp, sp_saw *p, SPFLOAT *in, SPFLOAT *out) 
{
    ...
    SPFLOAT *faust_in[] = {in};
    computesaw(dsp, 1, faust_in, faust_out);
