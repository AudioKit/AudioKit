...
extension AVAudioTime{
    ...
    /// AVAudioTime.extrapolateTime fails for host time valid times, use this instead. https://bugreport.apple.com/web/?problemID=34249528
    func extrapolateTimeShimmed(fromAnchor anchorTime: AVAudioTime) -> AVAudioTime{
        ...
        if isSampleTimeValid && sampleRate == anchorTime.sampleRate {
            ...
            let hostTime = secondsDiff >= 0 ? anchorTime.hostTime + UInt64(secondsDiff / ticksToSeconds) : anchorTime.hostTime - UInt64(-secondsDiff / ticksToSeconds)
            return AVAudioTime.init(hostTime: hostTime, sampleTime: sampleTime, atRate: anchorTime.sampleRate)
        }
        else{
            ...
            let audioTime = AVAudioTime.init(hostTime: hostTime, sampleTime: sampleTime, atRate: anchorTime.sampleRate)
            return audioTime
        }
    }
