...
open class AKFrequencyTracker: AKNode, AKToggleable, AKComponent {
    ...
    open dynamic var amplitude: Double {
        return Double(internalAU?.amplitude ?? 0) / Double(AKSettings.numberOfChannels) // Stereo Hack
    }

    /// Detected frequency
    open dynamic var frequency: Double {
        return Double(internalAU?.frequency ?? 0) * Double(AKSettings.numberOfChannels) // Stereo Hack
    }
