...
public class AKNoiseIsntrument: AKMidiInstrument{
    ...
    public init(white: Bool = true, numVoicesInit: Int) {
        //by default will init as white noise, pass in false to convert to pink
        self.white = white
        if(white){
            super.init(inst: AKWhiteNoise(), numVoicesInit: numVoicesInit)
        }else{
            super.init(inst: AKPinkNoise(), numVoicesInit: numVoicesInit)
        }
    }
