...
extension AKP {
    ...
    /** flatFrequencyResponseReverb: All-pass filter, often used for the creation of reverb modules. - This filter reiterates the input with an echo density determined by loop time.
 ...
 from its original amplitude).  Output will begin to appear immediately.
     ...
     */
    public static func flatFrequencyResponseReverb(
        input: AKParameter,
        reverbDuration: AKParameter = 0.5.ak,
        loopDuration: Double = 0.1
        ) -> AKParameter {
            return AKParameter("\(input)\(loopDuration) \(reverbDuration)allpass")
    }
}
