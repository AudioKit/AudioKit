...
//:
//: In this example, we've created a function `throttle`, that limits the amount
//: that a float can change per iteration, which is useful for preventing a "pop"
//: when parameters are changed. In this case, the limit is set to 0.001, which is
//: low enough that the effect is drawn out and easy to hear.
//:
...
let sporth = "(\(input) ((0 p) 0.001 (_throttle f)) 1000 100 pshift) dup"
