...
BlowHole ::BlowHole(StkFloat lowestFrequency) {
  ...
  thCoeff_ = (te * 2 * Stk::sampleRate() - 347.23) /
             (te * 2 * Stk::sampleRate() + 347.23);
  ...
  StkFloat rhCoeff = (zeta - 2 * Stk::sampleRate() * psi) /
                     (zeta + 2 * Stk::sampleRate() * psi);
...
void BlowHole ::setFrequency(StkFloat frequency) {
  ...
  if (frequency <= 0.0) {
    oStream_
        << "BlowHole::setFrequency: argument is less than or equal to zero!";
...
void BlowHole ::startBlowing(StkFloat amplitude, StkFloat rate) {
  if (amplitude <= 0.0 || rate <= 0.0) {
    oStream_ << "BlowHole::startBlowing: one or more arguments is less than or "
                "equal to zero!";
    handleError(StkError::WARNING);
    return;
  }
...
void BlowHole ::stopBlowing(StkFloat rate) {
  if (rate <= 0.0) {
    oStream_
        << "BlowHole::stopBlowing: argument is less than or equal to zero!";
    handleError(StkError::WARNING);
    return;
  }
...
void BlowHole ::controlChange(int number, StkFloat value) {
  ...
  if (Stk::inRange(value, 0.0, 128.0) == false) {
    oStream_ << "BlowHole::controlChange: value (" << value
             << ") is out of range!";
    handleError(StkError::WARNING);
    return;
  }
