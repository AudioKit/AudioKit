...
Bowed ::Bowed(StkFloat lowestFrequency) {
  ...
  // New body filter provided by Esteban Maestre (cascade of second-order
  // sections)
...
void Bowed ::setFrequency(StkFloat frequency) {
  ...
  neckDelay_.setDelay(baseDelay_ *
                      (1.0 - betaRatio_)); // bow to nut (finger) length
}

void Bowed ::startBowing(StkFloat amplitude, StkFloat rate) {
  if (amplitude <= 0.0 || rate <= 0.0) {
    oStream_ << "Bowed::startBowing: one or more arguments is less than or "
                "equal to zero!";
    handleError(StkError::WARNING);
    return;
  }
...
void Bowed ::controlChange(int number, StkFloat value) {
  ...
  if (value < 0 || (number != 101 && value > 128.0)) {
    oStream_ << "Bowed::controlChange: value (" << value
             << ") is out of range!";
    handleError(StkError::WARNING);
    return;
  }
