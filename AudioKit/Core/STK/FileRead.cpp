...
void FileRead::open(std::string fileName, bool typeRaw, unsigned int nChannels,
  ...
  if (typeRaw)
    ...
    else if (!strncmp(header, "FORM", 4) && (!strncmp(&header[8], "AIFF", 4) ||
                                             !strncmp(&header[8], "AIFC", 4)))
...
bool FileRead::getRawInfo(const char *fileName, unsigned int nChannels,
  ...
  fileSize_ =
      (long)filestat.st_size / sampleBytes / channels_; // length in frames
...
bool FileRead::getWavInfo(const char *fileName) {
  ...
  if (format_tag != 1 && format_tag != 3) { // PCM = 1, FLOAT = 3
    oStream_ << "FileRead: " << fileName
             << " contains an unsupported data format type (" << format_tag
             << ").";
    return false;
  }
...
bool FileRead::getSndInfo(const char *fileName) {
  ...
  if (format == 2)
    ...
    oStream_ << "FileRead: data format in file " << fileName
             << " is not supported.";
    return false;
  }
...
bool FileRead::getAifInfo(const char *fileName) {
  ...
  if (aifc == false) {
    ...
    if (!strncmp(id, "NONE", 4) || !strncmp(id, "sowt", 4)) {
      if (temp <= 8)
        ...
        dataType_ = STK_SINT32;
    } else if ((!strncmp(id, "fl32", 4) || !strncmp(id, "FL32", 4)) &&
               temp == 32)
      dataType_ = STK_FLOAT32;
  ...
  if (dataType_ == 0) {
    oStream_ << "FileRead: AIFF/AIFC file (" << fileName
             << ") has unsupported data type (" << id << ").";
    return false;
  }
...
bool FileRead::getMatInfo(const char *fileName) {
  ...
  if (strstr(head, "0")) {
    oStream_ << "FileRead: " << fileName
             << " appears to be a Version 4 MAT-file, which is not currently "
                "supported.";
    return false;
  }
  ...
  while (!doneParsing) {
    ...
    if (!haveSampleRate && rows == 1 &&
        columns == 1) { // Parse for sample rate.
      ...
      if (rows < columns) {
        ...
        oStream_ << "FileRead: Transpose the MAT-file array so that audio "
                    "channels fill matrix rows (not columns).";
        return false;
      }
      ...
      if (type == 1)
        ...
        oStream_ << "FileRead: The MAT-file array data format (" << type
                 << ") is not supported.";
        return false;
      }
...
void FileRead::read(StkFrames &buffer, unsigned long startFrame,
  ...
  if (nFrames == 0) {
    oStream_
        << "FileRead::read: StkFrames buffer size is zero ... no data read!";
    Stk::handleError(StkError::WARNING);
    return;
  }

  if (buffer.channels() != channels_) {
    oStream_ << "FileRead::read: StkFrames argument has incompatible number of "
                "channels!";
  ...
  if (dataType_ == STK_SINT16) {
    ...
    for (i = 0; i < nSamples; i++) {
      ...
      if (doNormalize) {
        buffer[i] =
            (StkFloat)temp * gain; // "gain" also  includes 1 / 256 factor.
      } else
        buffer[i] =
            (StkFloat)temp / 256; // right shift without affecting the sign bit
    }
  }
