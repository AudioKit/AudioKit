...
FileWrite::FileWrite(std::string fileName, unsigned int nChannels,
                     FILE_TYPE type, Stk::StkFormat format)
    : fd_(0) {
  this->open(fileName, nChannels, type, format);
}

FileWrite ::~FileWrite() { this->close(); }
...
void FileWrite ::open(std::string fileName, unsigned int nChannels,
  ...
  if (nChannels < 1) {
    oStream_
        << "FileWrite::open: then channels argument must be greater than zero!";
  ...
  if (format != STK_SINT8 && format != STK_SINT16 && format != STK_SINT24 &&
      format != STK_SINT32 && format != STK_FLOAT32 && format != STK_FLOAT64) {
    oStream_ << "FileWrite::open: unknown data type (" << format
             << ") specified!";
    handleError(StkError::FUNCTION_ARGUMENT);
  }
  dataType_ = format;
  ...
  if (fileType_ == FILE_RAW) {
    if (channels_ != 1) {
      oStream_ << "FileWrite::open: STK RAW files are, by definition, always "
                  "monaural (channels = "
               << nChannels << " not supported)!";
      handleError(StkError::FUNCTION_ARGUMENT);
    }
    ...
    oStream_ << "FileWrite::open: unknown file type (" << fileType_
             << ") specified!";
    handleError(StkError::FUNCTION_ARGUMENT);
  }
...
bool FileWrite ::setRawFile(std::string fileName) {
  ...
  if (dataType_ != STK_SINT16) {
    ...
    oStream_ << "FileWrite: using 16-bit signed integer data format for file "
             << fileName << '.';
...
bool FileWrite ::setSndFile(std::string fileName) {
  ...
  struct SndHeader hdr = {
      ".sn", 40, 0, 3, (SINT32)Stk::sampleRate(), 1, "Created by STK"};
  hdr.pref[3] = 'd';
...
bool FileWrite ::setMatFile(std::string fileName) {
  ...
  if (dataType_ != STK_FLOAT64) {
    ...
    oStream_ << "FileWrite: using 64-bit floating-point data format for file "
             << fileName << '.';
    handleError(StkError::DEBUG_PRINT);
  }

  struct MatHeader hdr;
  strcpy(hdr.heading, "MATLAB 5.0 MAT-file, Generated using the Synthesis "
                      "ToolKit in C++ (STK). By Perry R. Cook and Gary P. "
                      "Scavone.");
