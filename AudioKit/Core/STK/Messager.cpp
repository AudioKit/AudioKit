...
bool Messager ::setScoreFile(const char *filename) {
  if (data_.sources) {
    if (data_.sources == STK_FILE) {
      ...
      oStream_ << "Messager::setScoreFile: already reading realtime control "
                  "input ... cannot do scorefile input too!";
      handleError(StkError::WARNING);
    }
    return false;
  }

  if (!data_.skini.setFile(filename))
    return false;
...
bool Messager ::startStdInput() {
  if (data_.sources == STK_FILE) {
    oStream_ << "Messager::startStdInput: already reading a scorefile ... "
                "cannot do realtime control input too!";
    handleError(StkError::WARNING);
    return false;
  }
...
void midiHandler(double timeStamp, std::vector<unsigned char> *bytes,
                 void *ptr) {
  if (bytes->size() < 2)
    return;
...
bool Messager ::startMidiInput(int port) {
  if (data_.sources == STK_FILE) {
    oStream_ << "Messager::startMidiInput: already reading a scorefile ... "
                "cannot do realtime control input too!";
    handleError(StkError::WARNING);
    return false;
  }
  ...
  try {
    ...
    if (port == -1)
      data_.midi->openVirtualPort();
    else
      data_.midi->openPort((unsigned int)port);
  } catch (RtMidiError &error) {
    oStream_ << "Messager::startMidiInput: error creating RtMidiIn instance ("
             << error.getMessage() << ").";
    handleError(StkError::WARNING);
    return false;
  }
...
bool Messager ::startSocketInput(int port) {
  if (data_.sources == STK_FILE) {
    oStream_ << "Messager::startSocketInput: already reading a scorefile ... "
                "cannot do realtime control input too!";
    handleError(StkError::WARNING);
    return false;
  }
  ...
  if (data_.sources & STK_SOCKET) {
    oStream_
        << "Messager::startSocketInput: socket input thread already started.";
    handleError(StkError::WARNING);
    return false;
  }
  ...
  if (!socketThread_.start((THREAD_FUNCTION)&socketHandler, &data_)) {
    oStream_
        << "Messager::startSocketInput: unable to start socket input thread!";
    handleError(StkError::WARNING);
    return false;
  }
...
THREAD_RETURN THREAD_TYPE socketHandler(void *ptr) {
  ...
  while (data->sources & STK_SOCKET) {
    ...
    if (select(fd.back() + 1, &rmask, (fd_set *)0, (fd_set *)0, &timeout) <= 0)
      continue;

    // A file descriptor is set.  Check if there's a new socket connection
    // available.
