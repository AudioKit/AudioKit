...
MidiFileIn ::MidiFileIn(std::string fileName) {
  ...
  if (strncmp(chunkType, "MThd", 4) || (*length != 6)) {
    oStream_ << "MidiFileIn: file (" << fileName
             << ") does not appear to be a MIDI file!";
  ...
  if (format_ == 0 && *data != 1) {
    oStream_
        << "MidiFileIn: invalid number of tracks (>1) for a file format = 0!";
    handleError(StkError::FILE_ERROR);
  }
  nTracks_ = *data;
  ...
  if (format_ == 1 && !usingTimeCode_) {
    ...
    while (event.size()) {
      if ((event.size() == 6) && (event[0] == 0xff) && (event[1] == 0x51) &&
          (event[2] == 0x03)) {
...
unsigned long MidiFileIn ::getNextEvent(std::vector<unsigned char> *event,
  ...
  if (!usingTimeCode_) {
    if (isTempoEvent) {
      ...
      unsigned long value =
          (event->at(3) << 16) + (event->at(4) << 8) + event->at(5);
    ...
    if (format_ == 1) {
      ...
      if (trackCounters_[track] >= tempoEvent.count &&
          trackTempoIndex_[track] < tempoEvents_.size() - 1) {
...
unsigned long
MidiFileIn ::getNextMidiEvent(std::vector<unsigned char> *midiEvent,
                              unsigned int track) {
  ...
  while (midiEvent->size() && (midiEvent->at(0) >= 0xF0)) {
    ...
    // std::cout << "event byte = " << i << ", value = " <<
    // (int)midiEvent->at(i) << std::endl;
    ticks = getNextEvent(midiEvent, track);
  }

  // for ( unsigned int i=0; i<midiEvent->size(); i++ )
  // std::cout << "event byte = " << i << ", value = " << (int)midiEvent->at(i)
  // << std::endl;
