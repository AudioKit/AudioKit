...
void PRCRev ::setT60(StkFloat T60) {
  ...
  combCoefficient_[0] =
      pow(10.0, (-3.0 * combDelays_[0].getDelay() / (T60 * Stk::sampleRate())));
  combCoefficient_[1] =
      pow(10.0, (-3.0 * combDelays_[1].getDelay() / (T60 * Stk::sampleRate())));
}
...
StkFrames &PRCRev ::tick(StkFrames &frames, unsigned int channel) {
  ...
  if (channel >= frames.channels() - 1) {
    oStream_
        << "PRCRev::tick(): channel and StkFrames arguments are incompatible!";
    handleError(StkError::FUNCTION_ARGUMENT);
  }
#endif
...
StkFrames &PRCRev ::tick(StkFrames &iFrames, StkFrames &oFrames,
