...
void Plucked ::setFrequency(StkFloat frequency) {
  ...
  if (frequency <= 0.0) {
    oStream_
        << "Plucked::setFrequency: argument is less than or equal to zero!";
  ...
  StkFloat delay =
      (Stk::sampleRate() / frequency) - loopFilter_.phaseDelay(frequency);
...
void Plucked ::pluck(StkFloat amplitude) {
  ...
  for (unsigned long i = 0; i < delayLine_.getDelay(); i++)
    ...
    delayLine_.tick(0.6 * delayLine_.lastOut() +
                    pickFilter_.tick(noise_.tick()));
}
