...
void Saxofony ::setFrequency(StkFloat frequency) {
  ...
  if (frequency <= 0.0) {
    oStream_
        << "Saxofony::setFrequency: argument is less than or equal to zero!";
  ...
  StkFloat delay =
      (Stk::sampleRate() / frequency) - filter_.phaseDelay(frequency) - 1.0;
...
void Saxofony ::startBlowing(StkFloat amplitude, StkFloat rate) {
  if (amplitude <= 0.0 || rate <= 0.0) {
    oStream_ << "Saxofony::startBlowing: one or more arguments is less than or "
                "equal to zero!";
    handleError(StkError::WARNING);
    return;
  }
...
void Saxofony ::stopBlowing(StkFloat rate) {
  if (rate <= 0.0) {
    oStream_
        << "Saxofony::stopBlowing: argument is less than or equal to zero!";
    handleError(StkError::WARNING);
    return;
  }
...
void Saxofony ::controlChange(int number, StkFloat value) {
  ...
  if (Stk::inRange(value, 0.0, 128.0) == false) {
    oStream_ << "Saxofony::controlChange: value (" << value
             << ") is out of range!";
    handleError(StkError::WARNING);
    return;
  }
