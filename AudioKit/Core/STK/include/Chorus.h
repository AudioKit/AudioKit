...
inline StkFloat Chorus ::tick(StkFloat input, unsigned int channel) {
  ...
  delayLine_[0].setDelay(baseLength_ * 0.707 *
                         (1.0 + modDepth_ * mods_[0].tick()));
  delayLine_[1].setDelay(baseLength_ * 0.5 *
                         (1.0 - modDepth_ * mods_[1].tick()));
...
inline StkFrames &Chorus ::tick(StkFrames &frames, unsigned int channel) {
  ...
  for (unsigned int i = 0; i < frames.frames(); i++, samples += hop) {
    ...
    delayLine_[1].setDelay(baseLength_ * 0.5 *
                           (1.0 - modDepth_ * mods_[1].tick()));
    *samples =
        effectMix_ * (delayLine_[0].tick(*samples) - *samples) + *samples;
    samples++;
    *samples =
        effectMix_ * (delayLine_[1].tick(*samples) - *samples) + *samples;
  }
...
inline StkFrames &Chorus ::tick(StkFrames &iFrames, StkFrames &oFrames,
  ...
  for (unsigned int i = 0; i < iFrames.frames();
    ...
    delayLine_[1].setDelay(baseLength_ * 0.5 *
                           (1.0 - modDepth_ * mods_[1].tick()));
    *oSamples =
        effectMix_ * (delayLine_[0].tick(*iSamples) - *iSamples) + *iSamples;
    *(oSamples + 1) =
        effectMix_ * (delayLine_[1].tick(*iSamples) - *iSamples) + *iSamples;
  }
