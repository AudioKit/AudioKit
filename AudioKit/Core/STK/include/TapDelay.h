...
public:
  //! The default constructor creates a delay-line with maximum length of 4095
  //! samples and a single tap at delay = 0.
  /*!
  ...
  */
  ...
  /*!
    The valid range for each tap length is from 0 to the maximum delay-line
    length.
  */
  ...
  */
  ...
  //! Take a channel of the StkFrames object as inputs to the filter and write
  //! outputs back to the same object.
  /*!
    The StkFrames argument reference is returned.  The output
    values are ordered according to the tap positions set using the
    setTapDelays() function (no sorting is performed).  The StkFrames
...
inline StkFloat TapDelay ::lastOut(unsigned int tap) const {
  ...
  if (tap >= lastFrame_.size()) {
    oStream_ << "TapDelay::lastOut(): tap argument and number of taps are "
                "incompatible!";
...
inline StkFrames &TapDelay ::tick(StkFrames &frames, unsigned int channel) {
  ...
  if (channel >= frames.channels()) {
    oStream_ << "TapDelay::tick(): channel and StkFrames arguments are "
                "incompatible!";
    handleError(StkError::FUNCTION_ARGUMENT);
  }
  ...
  for (unsigned long i = 0; i < frames.frames();
  ...
  for (j = 0; j < outPoint_.size(); j++)
    lastFrame_[j] = *oSamples++;
  return frames;
}

inline StkFrames &TapDelay ::tick(StkFrames &iFrames, StkFrames &oFrames,
  ...
  if (oFrames.channels() < outPoint_.size()) {
    oStream_ << "TapDelay::tick(): number of taps > channels in output "
                "StkFrames argument!";
    handleError(StkError::FUNCTION_ARGUMENT);
  }
#endif
  ...
  for (unsigned long i = 0; i < iFrames.frames();
       i++, iSamples += iHop, oSamples += oHop) {
    inputs_[inPoint_++] = *iSamples * gain_;
    if (inPoint_ == inputs_.size())
      inPoint_ = 0;
