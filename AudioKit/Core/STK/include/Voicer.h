...
public:
  ...
  */
  ...
  //! Initiate a noteOn event with the given note number and amplitude and
  //! return a unique note tag.
  /*!
  ...
  */
  ...
  //! Send a noteOff to all voices having the given noteNumber and optional
  //! group (default group = 0).
  /*!
  ...
  */
  ...
  //! Send a frequency update message to all voices assigned to the optional
  //! group argument (default group = 0).
  /*!
    The \e noteNumber argument corresponds to a MIDI note number, though it is a
    floating-point value and can range beyond the normal 0-127 range.
  */
  ...
  */
  ...
  //! Send a pitchBend message to all voices assigned to the optional group
  //! argument (default group = 0).
  ...
  //! Send a controlChange to all instruments assigned to the optional group
  //! argument (default group = 0).
  ...
  */
  ...
  //! Mix one sample frame of all sounding voices and return the specified \c
  //! channel value.
  /*!
    The \c channel argument must be less than the number of output
    channels, which can be determined with the channelsOut() function
  ...
  */
  ...
  //! Fill the StkFrames argument with computed frames and return the same
  //! reference.
  /*!
...
protected:
  struct Voice {
    ...
    // Default constructor.
    Voice()
        : instrument(0), tag(0), noteNumber(-1.0), frequency(0.0), sounding(0),
          group(0) {}
  };
...
inline StkFloat Voicer ::tick(unsigned int channel) {
  ...
  for (unsigned int i = 0; i < voices_.size(); i++) {
    if (voices_[i].sounding != 0) {
      ...
      for (j = 0; j < voices_[i].instrument->channelsOut(); j++)
        lastFrame_[j] += voices_[i].instrument->lastOut(j);
    }
    if (voices_[i].sounding < 0)
      voices_[i].sounding++;
    if (voices_[i].sounding == 0)
