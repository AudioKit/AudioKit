...
Granulate ::Granulate(unsigned int nVoices, std::string fileName,
                      bool typeRaw) {
  this->setGrainParameters(); // use default values
  this->setRandomFactor();
  gStretch_ = 0;
...
void Granulate ::setGrainParameters(unsigned int duration,
                                    unsigned int rampPercent, int offset,
                                    unsigned int delay) {
  gDuration_ = duration;
  if (gDuration_ == 0) {
    gDuration_ = 1;
    oStream_ << "Granulate::setGrainParameters: duration argument cannot be "
                "zero ... setting to 1 millisecond.";
  ...
  if (gRampPercent_ > 100) {
    gRampPercent_ = 100;
    oStream_ << "Granulate::setGrainParameters: rampPercent argument cannot be "
                "greater than 100 ... setting to 100.";
    handleError(StkError::WARNING);
  }
...
void Granulate ::openFile(std::string fileName, bool typeRaw) {
  ...
  message << "Granulate::openFile: file = " << fileName
          << ", file frames = " << file.fileSize() << '.';
...
#if defined(_STK_DEBUG_)
  std::ostringstream message;
  message << "Granulate::setVoices: nVoices = " << nVoices
          << ", existing voices = " << grains_.size() << '.';
  handleError(message.str(), StkError::DEBUG_PRINT);
#endif
...
void Granulate ::calculateGrain(Granulate::Grain &grain) {
  ...
  while (grain.pointer >= data_.frames())
    grain.pointer -= data_.frames();
  if (grain.pointer < 0)
    grain.pointer = 0;
...
StkFloat Granulate ::tick(unsigned int channel) {
  ...
  if (channel >= data_.channels()) {
    oStream_ << "Granulate::tick(): channel argument and soundfile data are "
                "incompatible!";
  ...
  for (i = 0; i < grains_.size(); i++) {
    ...
    if (grains_[i].state > 0) {
      for (j = 0; j < nChannels; j++) {
        ...
        if (grains_[i].state == GRAIN_FADEIN ||
            grains_[i].state == GRAIN_FADEOUT) {
  ...
  if (stretchCounter_++ == gStretch_) {
    gPointer_++;
    if ((unsigned long)gPointer_ >= data_.frames())
      gPointer_ = 0;
    stretchCounter_ = 0;
  }
