...
public:
  //! Default constructor.
  BowTable(void)
      : offset_(0.0), slope_(0.1), minOutput_(0.01), maxOutput_(0.98){};
...
inline StkFloat BowTable ::tick(StkFloat input) {
  ...
  if (lastFrame_[0] < minOutput_)
    lastFrame_[0] = minOutput_;

  // Set maximum threshold
  if (lastFrame_[0] > maxOutput_)
    lastFrame_[0] = maxOutput_;
...
inline StkFrames &BowTable ::tick(StkFrames &frames, unsigned int channel) {
  ...
  if (channel >= frames.channels()) {
    oStream_ << "BowTable::tick(): channel and StkFrames arguments are "
                "incompatible!";
...
inline StkFrames &BowTable ::tick(StkFrames &iFrames, StkFrames &oFrames,
