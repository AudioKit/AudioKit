...
//: Another import line you will see in most playgrounds enables `XCPlayground`
//: functions for plotting, keeping the playground alive while audio plays, and more.
import XCPlayground


//: To reference a file, you may use AKAudioFile: Here, we load a file from the
//: resources folder (the files embeded in your application).
...
//: [looperman.com](http://www.looperman.com/) or [freesound.org](https://www.freesound.org/).
//:
...
//: `let variable = AKClass.init(input)` which has the added benefit of
//: providing better code completion and inline documentation.  This may not be
//: necessary as Xcode's support for Swift code completion improves.
...
//: Now that we are near the bottom of the screen (unless you have a majorly
//: tall monitor!) we'd like to call your attention to the playground controls
//: on the bottom left right below the navbar.
//:
//: The first button toggles the console log which can be useful to look at when
//: things go wrong. The second button is your play / stop button which is useful
//: to control playback of the audio in the playground. If you click and hold on
//: this button you will get a pop-up that will allow you choose between
