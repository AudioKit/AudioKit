...
// AKFunctionTable represents a simple one-dimensional table of float values, addressable
// using a normalized fractional index, [0.0, 1.0), with or without wraparound.
...
// See class AKFunctionTableOscillator.
...
#define DEFAULT_AKWAVETABLE_SIZE 256

struct AKFunctionTable
{
    float *pWaveTable;
    int nTableSize;
    
    AKFunctionTable() : pWaveTable(0), nTableSize(0) {}
    ~AKFunctionTable() { deinit(); }
    
    void init(int tableLength=DEFAULT_AKWAVETABLE_SIZE);
    void deinit();
    
    // functions for use by class AKFunctionTableOscillator
    void triangle();
    void sawtooth();
    void sinusoid();
...
// AKFunctionTableOscillator implements a simple wavetable-based oscillator. Small table sizes (as small
...
struct AKFunctionTableOscillator
{
    double sampleRateHz;
    float phase;
    float phaseDelta;   // normalized frequency: cycles per sample
    AKFunctionTable waveTable;
    
    ~AKFunctionTableOscillator() { deinit(); }
...
// AKWaveShaper wraps a AKFunctionTable and provides saved scale and offset parameters for both
...
struct AKWaveShaper
{
    ...
    AKWaveShaper() : xScale(1.0f), xOffset(0.0f), yScale(1.0f), yOffset(0.0f) {}
    ~AKWaveShaper() { deinit(); }
    void deinit() { waveTable.deinit(); }
    
    void init(int tableLength=256) { waveTable.init(tableLength); }
