...
- Accessing and editing MIDI note data using AKAppleSequencer and AKMusicTrack
- Exporting MIDI files from the contents of AKAppleSequencer 


## Creating a new sequence from a MIDI file

AKAppleSequencer can be initialized by either a filename or URL, or an existing AKAppleSequencer can have a new internal sequence created by calling ```loadMIDIFile()```.
...
Calling AKAppleSequencer's ```genData()``` will create the data that can be easily written to a URL and exported as a MIDI file.
