...
class ViewController: UIViewController {
    ...
    func genNewMelodicSequence(stepSize:Float = 1/8, minor:Bool = false, clear:Bool = true){
        ...
        for (var i:Int = 0; i < numSteps; i++){
            if(random(0,16) > 12){
                ...
                seq.tracks[0].addNote(noteToAdd, vel: 100, position: Double(step), dur: 1)
            }
        }
        seq.setLength(seqLen)
    }
    ...
    func genBDSeq(stepSize:Float = 1, clear:Bool = true){
        ...
        for (var i:Int = 0; i < numSteps; i++){
            let step = (Float(i) * stepSize)
            seq.tracks[1].addNote(60, vel: 100, position: Double(step), dur: 1)
        }
    }
    ...
    func genSDSeq(stepSize:Float = 1, clear:Bool = true){
        ...
        let numSteps = Int(Float(seqLen)/stepSize)
        for (var i:Int = 1; i < numSteps; i += 2){
            let step = (Float(i) * stepSize)
            seq.tracks[2].addNote(60, vel: 80, position: Double(step), dur: 1)
        }
    }
    ...
    func genSDGhostSeq(stepSize:Float = 1/8,clear:Bool = true){
        ...
        for (var i:Int = 0; i < numSteps; i++){
            if(random(0,16) > 14.0){
                let step = (Float(i) * stepSize)
                seq.tracks[3].addNote(60, vel: Int(random(1,66)), position: Double(step), dur: 0.1)
            }
        }
        seq.setLength(seqLen)
    }
