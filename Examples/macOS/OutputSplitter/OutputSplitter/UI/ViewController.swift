...
class ViewController: NSViewController {
    ...
    func setOutputDeviceSelectors () {
        ...
        output1Selector.addItems(withObjectValues: outputDevices.filter { $0.deviceID != Application.output2?.device.deviceID}.map { $0.name })
        output2Selector.removeAllItems()
        output2Selector.addItems(withObjectValues: outputDevices.filter { $0.deviceID != Application.output1?.device.deviceID}.map { $0.name })
