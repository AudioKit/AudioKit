...
FFTReal<DT>::FFTReal(long length)
    : _length(length), _nbr_bits(FFTReal_get_next_pow2(length)), _br_lut(),
      _trigo_lut(), _buffer(length), _trigo_osc() {
...
template <class DT> long FFTReal<DT>::get_length() const { return (_length); }
