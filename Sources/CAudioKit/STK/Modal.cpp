...
Modal ::Modal(unsigned int modes) : nModes_(modes) {
  if (nModes_ == 0) {
...
void Modal ::setRatioAndRadius(unsigned int modeIndex, StkFloat ratio,
                               StkFloat radius) {
  if (modeIndex >= nModes_) {
    oStream_ << "Modal::setRatioAndRadius: modeIndex parameter is greater than "
                "number of modes!";
    handleError(StkError::WARNING);
    return;
  }
  ...
  if (ratio * baseFrequency_ < nyquist) {
    ...
    temp = ratio;
    while (temp * baseFrequency_ > nyquist)
      temp *= 0.5;
    ratios_[modeIndex] = temp;
#if defined(_STK_DEBUG_)
    oStream_ << "Modal::setRatioAndRadius: aliasing would occur here ... "
                "correcting.";
...
void Modal ::setModeGain(unsigned int modeIndex, StkFloat gain) {
  if (modeIndex >= nModes_) {
    oStream_ << "Modal::setModeGain: modeIndex parameter is greater than "
                "number of modes!";
    handleError(StkError::WARNING);
    return;
  }
