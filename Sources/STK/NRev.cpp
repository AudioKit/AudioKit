...
NRev ::NRev(StkFloat T60) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    combCoefficient_[i] =
        pow(10.0, (-3 * lengths[i] / (T60 * Stk::sampleRate())));
  }
...
void NRev ::setT60(StkFloat T60) {
  ...
  for (int i = 0; i < 6; i++)
    combCoefficient_[i] = pow(
        10.0, (-3.0 * combDelays_[i].getDelay() / (T60 * Stk::sampleRate())));
}
...
StkFrames &NRev ::tick(StkFrames &frames, unsigned int channel) {
  ...
  if (channel >= frames.channels() - 1) {
    oStream_
        << "NRev::tick(): channel and StkFrames arguments are incompatible!";
    handleError(StkError::FUNCTION_ARGUMENT);
  }
#endif
...
StkFrames &NRev ::tick(StkFrames &iFrames, StkFrames &oFrames,
