...
void Whistle ::setFrequency(StkFloat frequency) {
  ...
  if (frequency <= 0.0) {
    oStream_
        << "Whistle::setFrequency: parameter is less than or equal to zero!";
...
void Whistle ::startBlowing(StkFloat amplitude, StkFloat rate) {
  if (amplitude <= 0.0 || rate <= 0.0) {
    oStream_ << "Whistle::startBlowing: one or more arguments is less than or "
                "equal to zero!";
    handleError(StkError::WARNING);
    return;
  }
...
StkFloat Whistle ::tick(unsigned int) {
  ...
  if (--subSampCount_ <= 0) {
    ...
    //			+ (fippleFreqMod_ * 0.25) - (fippleFreqMod_ * temp) //
    //fippleModulation
    //			- (blowFreqMod_) + (blowFreqMod_ * envOut); //
    //blowingModulation
    // short form of above
    tempFreq =
        1.0 + fippleFreqMod_ * (0.25 - temp) + blowFreqMod_ * (envOut - 1.0);
...
void Whistle ::controlChange(int number, StkFloat value) {
  ...
  if (Stk::inRange(value, 0.0, 128.0) == false) {
    oStream_ << "Whistle::controlChange: value (" << value
             << ") is out of range!";
    handleError(StkError::WARNING);
    return;
  }
