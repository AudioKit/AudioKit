...
public:
  ...
  //! Set the mixture of input and "effected" levels in the output (0.0 = input
  //! only, 1.0 = effect only).
...
inline void Effect ::setEffectMix(StkFloat mix) {
  if (mix < 0.0) {
    oStream_ << "Effect::setEffectMix: mix parameter is less than zero ... "
                "setting to zero!";
    handleError(StkError::WARNING);
    effectMix_ = 0.0;
  } else if (mix > 1.0) {
    oStream_ << "Effect::setEffectMix: mix parameter is greater than 1.0 ... "
                "setting to one!";
    handleError(StkError::WARNING);
    effectMix_ = 1.0;
  } else
