...
protected:
  ...
  */
  ...
  StkFloat env[2];  // Coefficients for the linear interpolation when modifying
                    // the output samples
  ...
  // Coefficient delay line that could be used for a dynamic calculation of the
  // pitch
  // Delay* coeffLine_;
};
...
inline void LentPitShift::process() {
  ...
  lastPeriod_ =
      tMax_ + 1; // Storage of the lowest local minimum under the threshold
  ...
  for (delay_ = 1; delay_ <= tMax_; delay_++) {
    ...
    // Look for a minimum
    if (dpt[delay_ - 1] - dpt[delay_ - 2] < 0 &&
        dpt[delay_] - dpt[delay_ - 1] > 0) {
  ...
  // We loop for all the frames of length lastPeriod_ presents between inputPtr
  // and tMax_
  for (; inputPtr < (int)(tMax_ - lastPeriod_); inputPtr += lastPeriod_) {
    ...
    while (outputPtr < inputPtr) {
      ...
      N = 2 * tMax_ - (unsigned long)floor(outputPtr + tMax_) + lastPeriod_ -
          1; // New writing pointer
...
inline StkFrames &LentPitShift ::tick(StkFrames &frames, unsigned int channel) {
  ...
  if (channel >= frames.channels()) {
    oStream_ << "LentPitShift::tick(): channel and StkFrames arguments are "
                "incompatible!";
...
inline StkFrames &LentPitShift ::tick(StkFrames &iFrames, StkFrames &oFrames,
