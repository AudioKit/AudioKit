...
int sporth_pan2(sporth_stack *stack, void *ud)
{
    ...
    switch(pd->mode) {
...
#ifdef DEBUG_MODE
            plumber_print(pd, "pan2: Creating\n");
#endif

            sp_pan2_create(&pan2);
            plumber_add_ugen(pd, SPORTH_PAN, pan2);
            if(sporth_check_args(stack, "ff") != SPORTH_OK) {
                plumber_print(pd,"Not enough arguments for pan2\n");
                stack->error++;
...
#ifdef DEBUG_MODE
            plumber_print(pd, "pan2: Initialising\n");
#endif
            pan = sporth_stack_pop_float(stack);
            in = sporth_stack_pop_float(stack);
            pan2 = (sp_pan2*)pd->last->ud;
        ...
        case PLUMBER_DESTROY:
            ...
            break;
        default:
            plumber_print(pd, "pan2: Unknown mode!\n");
            break;
    }
    return PLUMBER_OK;
}
