...
static int slist_parse(slist_d *sl, const char *filename, uint32_t size)
{
    ...
    ssize_t read = 0;
    char *line = NULL;
    sl-> list = (char**)malloc(sizeof(char *) * size);
    if(fp == NULL) {
...
int sporth_slist(sporth_stack *stack, void *ud)
{
    ...
    switch(pd->mode){
        case PLUMBER_CREATE:
            sl = (slist_d*)malloc(sizeof(slist_d));
            sl->list = NULL;
...
int sporth_sget(sporth_stack *stack, void *ud)
{
    ...
    switch(pd->mode){
        case PLUMBER_CREATE:
            ...
            if(plumber_ftmap_search_userdata((plumber_data *)pd,(const char *)ftname, (void**)&sl) !=
                    PLUMBER_OK) {
