...
class AmplitudeEnvelopeTests: XCTestCase {
    func testAttack() {
        ...
        let envelope = AmplitudeEnvelope(input, attackDuration: 0.1234)
    ...
    func testParameters() {
        let engine = AudioEngine()
        let input = Oscillator()
        let envelope = AmplitudeEnvelope(input, attackDuration: 0.1234, decayDuration: 0.234, sustainLevel: 0.345)
        engine.output = envelope
        input.play()
        envelope.start()
